...
import './QuanLyNguoiDungGV.css';

const QuanLyNguoiDungGV = () => {
  ...
  useEffect(() => {
    userService.getLecturers()
      ...
      .catch(err => console.error('Loi tai giang vien:', err))
      .finally(() => setLoading(false));
  }, []);

  const filteredData = data.filter(item => {
    const matchSearch = item.ten.toLowerCase().includes(tuKhoa.toLowerCase()) ||
                        item.maGV.toLowerCase().includes(tuKhoa.toLowerCase());
  ...
  const handleDelete = async (id) => {
    try {
      ...
      console.error('Loi xoa giang vien:', err);
  ...
  return (
    <div className="khung-quan-ly-gv-tong">
      <ThongKeNguoiDung tong={data.length} isLecturerView={true} />
      <SearchBarGroupAdmin
        tuKhoa={tuKhoa} setTuKhoa={setTuKhoa}
        trangThai={trangThai} setTrangThai={setTrangThai}
        isStudentView={true}
      />
      <div className="vung-chua-bang-gv-trang">
        <BangNguoiDung
          data={currentItems}
...
export default QuanLyNguoiDungGV;